...
const Sidebar = ({ user, currentView, onNavigate, onLoadQuery, onNewQuery }) => {
  ...
  return (
    <aside className="w-64 bg-slate-900 border-r border-slate-800 h-screen flex flex-col transition-all duration-300 z-30">
      {/* Header del Sidebar */}
      <div className="p-4 flex flex-col gap-4">
        ...
        <button
          onClick={onNewQuery}
          className="flex items-center gap-2 bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 rounded-xl px-4 py-2.5 transition-colors font-medium shadow-sm"
        >
      ...
      <div className="flex-1 overflow-y-auto px-3 py-2">
        ...
        <ul className="space-y-1">
          {recentQueries.map((query) => (
            <li key={query.id}>
              <button
                onClick={() => onLoadQuery(query.resultData)}
                className="w-full text-left px-3 py-2 rounded-lg text-sm text-slate-300 hover:bg-slate-800 transition-colors truncate flex items-center gap-2"
              >
